// <autogenerated>
//   This file was generated by dddappp code generator.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>

module rooch_test_proj1::order_item_quantity_updated {

    use moveos_std::object::ObjectID;
    use rooch_test_proj1::order::{Self, OrderItemQuantityUpdated};
    use std::string::String;

    public fun id(order_item_quantity_updated: &OrderItemQuantityUpdated): ObjectID {
        order::order_item_quantity_updated_id(order_item_quantity_updated)
    }

    public fun order_id(order_item_quantity_updated: &OrderItemQuantityUpdated): String {
        order::order_item_quantity_updated_order_id(order_item_quantity_updated)
    }

    public fun product_obj_id(order_item_quantity_updated: &OrderItemQuantityUpdated): ObjectID {
        order::order_item_quantity_updated_product_obj_id(order_item_quantity_updated)
    }

    public fun quantity(order_item_quantity_updated: &OrderItemQuantityUpdated): u64 {
        order::order_item_quantity_updated_quantity(order_item_quantity_updated)
    }

}
