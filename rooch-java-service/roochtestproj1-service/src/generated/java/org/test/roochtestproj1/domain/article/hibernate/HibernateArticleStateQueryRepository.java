...
package org.test.roochtestproj1.domain.article.hibernate;
...
import org.test.roochtestproj1.specialization.hibernate.*;
...
public class HibernateArticleStateQueryRepository implements ArticleStateQueryRepository {
    ...
    public ReferenceState getReference(String articleId, BigInteger referenceNumber) {
        ArticleReferenceId entityId = new ArticleReferenceId(articleId, referenceNumber);
        return (ReferenceState) getCurrentSession().get(AbstractReferenceState.SimpleReferenceState.class, entityId);
    }

    @Transactional(readOnly = true)
    public Iterable<ReferenceState> getReferences(String articleId, org.dddml.support.criterion.Criterion filter, List<String> orders) {
        Criteria criteria = getCurrentSession().createCriteria(AbstractReferenceState.SimpleReferenceState.class);
        org.hibernate.criterion.Junction partIdCondition = org.hibernate.criterion.Restrictions.conjunction()
            .add(org.hibernate.criterion.Restrictions.eq("articleReferenceId.articleId", articleId))
            ;
