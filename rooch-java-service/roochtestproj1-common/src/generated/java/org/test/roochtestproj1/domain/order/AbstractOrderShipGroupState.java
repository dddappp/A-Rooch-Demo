...
import org.test.roochtestproj1.domain.order.OrderShipGroupEvent.*;
...
public abstract class AbstractOrderShipGroupState implements OrderShipGroupState.SqlOrderShipGroupState, Saveable {
        ...
        if (s.getOrderItemShipGroupAssociations() != null) {
            ...
            if (iterable != null) {
                for (OrderItemShipGroupAssociationState ss : iterable) {
                    OrderItemShipGroupAssociationState thisInnerState = ((EntityStateCollection.ModifiableEntityStateCollection<String, OrderItemShipGroupAssociationState>)this.getOrderItemShipGroupAssociations()).getOrAdd(ss.getProductObjId());
        ...
        if (s.getOrderItemShipGroupAssociations() != null) {
            if (s.getOrderItemShipGroupAssociations() instanceof EntityStateCollection.ModifiableEntityStateCollection) {
                ...
                if (s.getOrderItemShipGroupAssociations().isAllLoaded()) {
                    Set<String> removedStateIds = new HashSet<>(this.getOrderItemShipGroupAssociations().stream().map(i -> i.getProductObjId()).collect(java.util.stream.Collectors.toList()));
                    s.getOrderItemShipGroupAssociations().forEach(i -> removedStateIds.remove(i.getProductObjId()));
    ...
    class SimpleOrderItemShipGroupAssociationStateCollection implements EntityStateCollection.ModifiableEntityStateCollection<String, OrderItemShipGroupAssociationState> {
        ...
        @Override
        public OrderItemShipGroupAssociationState get(String productObjId) {
            return protectedOrderItemShipGroupAssociations.stream().filter(
                            e -> e.getProductObjId().equals(productObjId))
        ...
        @Override
        public OrderItemShipGroupAssociationState getOrAdd(String productObjId) {
            OrderItemShipGroupAssociationState s = get(productObjId);
            if (s == null) {
                OrderItemShipGroupAssociationId globalId = new OrderItemShipGroupAssociationId(getOrderShipGroupId().getOrderId(), getOrderShipGroupId().getShipGroupSeqId(), productObjId);
